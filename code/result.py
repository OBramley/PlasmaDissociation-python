...
def process_results(rep,bondarr):
   
    # # 2. detect dissociation
    detect_dissociation(bondarr,rep)
    # 3. Find fragments.
    fragments(rep)
    # 4. compile dissociation data
    compile_results(rep)
    # 5. Compile fragment data
    combine_fragments(rep)
    # 6. Specifics
    spc.specifics('../results/bondarr.txt')
    #  7. graphs
    write_graphs_json(rep)
...
def compile_results(rep):

    with open('../rep-'+str(rep)+'/output/dissociation.out', "r") as f:
        lines = f.readlines()
    EXDIR= os.getcwd()
    print(EXDIR)
...
def combine_fragments(rep):
    if os.path.exists("../results/fragments.out"):
       combine_fragment_counts("../rep-"+str(rep)+"/output/fragments.out", "../results/fragments.out", "../results/fragments.out")
    else:
        shutil.copy2("../rep-"+str(rep)+"/output/fragments.out","../results")
...
def write_graphs_json(rep):
    ...
    file_path = "../rep-"+str(rep)+"/Geometry"
    with open(file_path, 'r') as file:
        lines = file.readlines()
